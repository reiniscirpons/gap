                        ...
                        Obj rel)    // handle of the relator
{
    ...
    if (lp == rp + 1 && INT_INTOBJ(ELM_PLIST(ELM_PLIST(rel, lp), lc)) != rc) {
        return True;
    }
    else
        return False;
...
*/
static void HandleCoinc(Int cos1, Int cos2)
{
    Obj * ptTable;    // pointer to the coset table
    Obj * ptNext;
    Obj * ptPrev;
    ...
    while (firstCoinc != 0) {
        ...
        cos1 = INT_INTOBJ(ptPrev[firstCoinc]);
        cos2 = firstCoinc;
                ...
                if (lp == rp + 1) {
                    if (INT_INTOBJ(ELM_PLIST(ptRel[lp], lc)) != rc) {
                        if (INT_INTOBJ(ELM_PLIST(ptRel[lp], lc)) > 0) {
                            HandleCoinc(INT_INTOBJ(ELM_PLIST(ptRel[lp], lc)),
                                        rc);
                        }
                        else if (INT_INTOBJ(ELM_PLIST(ptRel[rp], rc)) > 0) {
                            HandleCoinc(INT_INTOBJ(ELM_PLIST(ptRel[rp], rc)),
                                        lc);
                        }
                        else {
...
static Obj FuncStandardizeTableC(Obj self, Obj table, Obj stan)
{
    ...
    for (j = 1; j <= nrgen * 2; j++) {
        if (!IS_PLIST(ptTable[j])) {
            ErrorQuit("<table>[%d] must be a plain list (not a %s)", (Int)j,
...
*/
static Int AddCosetFactor2(Int factor)
{
                                   ...
                                   Obj s2)    // label stack
{
    ...
    while ((ok == 1) && (j > 0)) {
        ...
        x = s2a[j];    // edge label
        j--;
        rx = ELM_PLIST(r, x);    // Look at all relations which start with x
        l = LEN_PLIST(rx);
        i = 1;
        while ((ok == 1) && (i <= l)) {
            ok = RelatorScan(
                ...
                ELM_PLIST(
                    rx,
                    i));    // push first relation through table t at vertex d
            if (ok == 2) {    // 2 is code for when we were missing a single
                              // edge, so we define it
                j++;
                if (j > sd) {    // Ah, manual memory management
                    sd = 2 * sd;
                    GROW_PLIST(s1, sd);
                    SET_LEN_PLIST(s1, sd);
                    CHANGED_BAG(s1);
                ...
                s1a[j] = ret1;    // globals that store return value of
                                  // RelatorScan if it defines a new edge
                s2a[j] = ret2;
                ok = 1;
            }
            i++;
        ...
        // Same thing for reverse relation?
...
static Obj FuncLOWINDEX_PREPARE_RELS(Obj self, Obj r)    // rels
{
    ...
    for (i = 1; i <= LEN_PLIST(r); i++) {
        ...
        for (j = 1; j <= LEN_PLIST(ri); j++) {
            ...
            for (k = 1; k <= l; k++)
                rp[k] = INT_INTOBJ(
                    (Obj)rp[k]);    // convert relator entries to C-integers
            // change type
                             ...
                             Obj result)    // result list
{
    ...
    while ((i <= r) &&
           ((ff = INT_INTOBJ(ELM_PLIST(
                 ELM_PLIST(c, INT_INTOBJ(ELM_PLIST(w, i)) + offset), f))) !=
            0)) {
    ...
    while ((j >= i) &&
           ((bb = INT_INTOBJ(ELM_PLIST(
                 ELM_PLIST(c, -INT_INTOBJ(ELM_PLIST(w, j)) + offset), b))) !=
            0)) {
